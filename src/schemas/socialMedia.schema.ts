...
export const createSocialMediaSchema = z.object({
  ...
  icon: z.string().url('Format URL icon tidak valid').min(1, 'URL icon wajib diisi'),
  link: z.string().url('Format URL link tidak valid').min(1, 'URL link wajib diisi'),
});
...
export const updateSocialMediaSchema = z.object({
  ...
  icon: z.string().url('Format URL icon tidak valid').min(1, 'URL icon wajib diisi').optional(),
  link: z.string().url('Format URL link tidak valid').min(1, 'URL link wajib diisi').optional(),
});
